...
'''
def writingCatsAndDogs():
    output_file_1 = open('cats.txt', 'w')
    output_file_2 = open('dogs.txt', 'w')
    lst_outputs = [output_file_1, output_file_2]
...
'''while True:
    try:
        ...
        print(f'sum: {first_digit + second_digit}')
        break'''
...
That is, create a dictionary of student data in the following format: 
...
# gradebook_dict = {'students': [{'first_name': 'Manuel', 'last_name': 'Turizo', 'exam1': 88, 'exam2': 98, 'exam3': 100}]}
...
data = json.load(input_file) #cursor at the bottom of the line
for student_dict in data['students']:
    print(f'First name: {student_dict['first_name']} | Last name: {student_dict['last_name']}')
